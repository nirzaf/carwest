 ...
 MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
            {
                try
                {
                    ...
                    else
                    {
                        conn.Close();
                        conn.Open();
                        new SqlCommand("insert into ExpensesAccounts values ('" + number.Text + "','" + name.Text + "','" + subAcount + "')", conn).ExecuteNonQuery();
                        conn.Close();
        ...
        private void number_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                conn.Open();
                reader = new SqlCommand("select * from expensesAccounts ", conn).ExecuteReader();
                while (reader.Read())
                {
