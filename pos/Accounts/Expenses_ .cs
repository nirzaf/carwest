...
namespace pos
{
    public partial class Expenses_ : Form
    {
        public Expenses_(Form home, String user)
        {
            InitializeComponent();
            this.home = home;
            this.user = user;
        ...
        private void update(string id)
        {
            try
            {
                ...
                if (reader.Read())
                {
                    ...
                    new SqlCommand("update DepreciationAccounts set name='" + name.Text + "',subAcount='" + subAcount + "' where id='" + id + "'", conn).ExecuteNonQuery();
                    conn.Close();
                }
                else
                {
        ...
        public Boolean loadCustomer(string id)
        {
            try
            {
                db.setCursoerWait();
                conn.Open();
                reader = new SqlCommand("select * from DepreciationAccounts where id='" + id + "'", conn).ExecuteReader();
                if (reader.Read())
                {
                    states = true;
        ...
        private void loadAcounts()
        {
            try
            {
                ...
                conn.Open();
                reader = new SqlCommand("select id,name from DepreciationAccounts", conn).ExecuteReader();
                while (reader.Read())
                {
 ...
 MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
            {
                try
                {
                    ...
                    else
                    {
                        conn.Close();
                        conn.Open();
                        new SqlCommand("insert into DepreciationAccounts values ('" + number.Text + "','" + name.Text + "','" + subAcount + "')", conn).ExecuteNonQuery();
                        conn.Close();

                        conn.Open();
                        new SqlCommand("insert into accounts values('" + number.Text + "','" + name.Text + "','" + "DEPRECIATION" + "')", conn).ExecuteNonQuery();
                        conn.Close();
                    }
        ...
        private void number_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                conn.Open();
                reader = new SqlCommand("select * from DepreciationAccounts ", conn).ExecuteReader();
                while (reader.Read())
                {
