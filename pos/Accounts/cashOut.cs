...
namespace pos
{
    public partial class cashOut : Form
    {
        ...
        public cashOut(Form form, string user)
        {
            InitializeComponent();
            formH = form;
            userH = user;
        ...
        private void save()
        {
            try
            {
                ...
                else
                {
                    ...
                    if (comboBox1.SelectedIndex == 0 || comboBox1.SelectedIndex == 1 || comboBox1.SelectedIndex == 2 || comboBox1.SelectedIndex == 3 || comboBox1.SelectedIndex == 0)
                    {
                        conn.Open();
                        new SqlCommand("insert into receipt values('" + tempID + "','" + date.Value + "','" + "CASH OUT - NON EX" + "','" + comboBox1.SelectedItem.ToString() + "','" + new amountByName().setAmountName(amountCash.Text) + "','" + amountCash.Text + "','" + payee.Text + "','" + "" + "','" + "" + "','" + userH + "','" + DateTime.Now + "')", conn).ExecuteReader();
                        conn.Close();
                    }
                    else
                    {
                        conn.Open();
                        new SqlCommand("insert into receipt values('" + tempID + "','" + date.Value + "','" + "CASH OUT" + "','" + comboBox1.SelectedItem.ToString() + "','" + new amountByName().setAmountName(amountCash.Text) + "','" + amountCash.Text + "','" + payee.Text + "','" + "" + "','" + "" + "','" + userH + "','" + DateTime.Now + "')", conn).ExecuteReader();
                        conn.Close();
                    }

                    if (comboBox1.SelectedIndex == 6 || comboBox1.SelectedIndex == 7 || comboBox1.SelectedIndex == 8 || comboBox1.SelectedIndex == 9)
                    {
                    }
                    else
                    {
                        conn2.Open();
                        new SqlCommand("insert into cashSummery values('" + comboBox1.SelectedItem.ToString() + " " + payee.Text + "/" + tempID + "','" + "CASH OUT" + "','" + Double.Parse(amountCash.Text) + "','" + date.Value.ToString() + "','" + userH + "')", conn2).ExecuteNonQuery();
                        conn2.Close();
                    }
        ...
        private void load()
        {
            try
            {
                ...
                conn.Open();
                reader = new SqlCommand("select * from cashSummery where date='" + date.Value.ToShortDateString() + "' and remark='" + "CASH OUT" + "'", conn).ExecuteReader();
                while (reader.Read())
                {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.ColumnIndex == 3)
                {
                    ...
                    conn.Open();
                    new SqlCommand("delete from cashSummery where date='" + date.Value.ToShortDateString() + "' and remark='" + "CASH OUT" + "'", conn).ExecuteNonQuery();
                    conn.Close();
                    //for (int i = 0; i < dataGridView1.Rows.Count; i++)
                    //{
                    //    conn.Open();
                    //    new SqlCommand("insert into cashSummery values('" + dataGridView1.Rows[i].Cells[0].Value + "','" + "EXPENCES-MANUAL" + "','" + dataGridView1.Rows[i].Cells[1].Value + "','" + date.Value.ToShortDateString() + "','" + userH + "')", conn).ExecuteNonQuery();
                    //    conn.Close();
                    //}
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                dataGridView1.Rows.Clear();
                conn.Open();
                reader = new SqlCommand("select * from cashSummery where date between '" + dateFrom.Value.ToShortDateString() + "' and  '" + dateTo.Value.ToShortDateString() + "' and remark='" + "CASH OUT" + "'", conn).ExecuteReader();
                while (reader.Read())
                {
        ...
        private void button4_Click_1(object sender, EventArgs e)
        {
            ...
            {
                db.setCursoerWait();
                new invoicePrint().setprintCasOut(conn, reader, dateTo.Value.ToShortDateString(), dateFrom.Value.ToShortDateString());
