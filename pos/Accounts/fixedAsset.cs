...
namespace pos
{
    ...
    {
        public fixedAsset(Form home, String user)
        {
            InitializeComponent();
            this.home = home;
            this.user = user;
        ...
        void update(string id)
        {
            try
            {
                ...
                if (reader.Read())
                {
                    ...
                    new SqlCommand("update fixedAssets set name='" + name.Text + "',openingBalance='" + amount.Text + "',openingDate='" + date.Value + "' where id='" + id + "'", conn).ExecuteNonQuery();
                    conn.Close();
                    conn.Open();
 ...
 MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
        ...
        private void number_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                conn.Open();
                reader = new SqlCommand("select * from fixedAssets ", conn).ExecuteReader();
                while (reader.Read())
                {
