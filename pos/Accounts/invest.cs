    ...
    public partial class invest : Form
    {
        ...
        private void load()
        {
            try
            {
                ...
                conn.Open();
                reader = new SqlCommand("select * from cashSummery where date='" + date.Value.ToShortDateString() + "' and remark='" + "INVEST-MANUAL" + "'", conn).ExecuteReader();
                while (reader.Read())
                {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.ColumnIndex == 3)
                {
                    ...
                    conn.Open();
                    new SqlCommand("delete from cashSummery where date='" + date.Value.ToShortDateString() + "' and remark='" + "INVEST-MANUAL" + "'", conn).ExecuteNonQuery();
                    conn.Close();
                    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                    {
                        conn.Open();
                        new SqlCommand("insert into cashSummery values('" + dataGridView1.Rows[i].Cells[0].Value + "','" + "INVEST-MANUAL" + "','" + dataGridView1.Rows[i].Cells[1].Value + "','" + date.Value.ToShortDateString() + "','" + userH + "')", conn).ExecuteNonQuery();
                        conn.Close();
                    }
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                dataGridView1.Rows.Clear();
                conn.Open();
                reader = new SqlCommand("select * from cashSummery where date between '" + dateFrom.Value.ToShortDateString() + "' and  '" + dateTo.Value.ToShortDateString() + "' and remark='" + "INVEST-MANUAL" + "'", conn).ExecuteReader();
                while (reader.Read())
                {
