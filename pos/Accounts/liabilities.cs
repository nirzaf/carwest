...
namespace pos
{
    public partial class liabilities : Form
    {
        public liabilities(Form home, String user)
        {
            InitializeComponent();
            this.home = home;
            this.user = user;
        ...
        private void update(string id)
        {
            try
            {
                ...
                if (reader.Read())
                {
                    ...
                    new SqlCommand("update liabilities set name='" + name.Text + "',openingBalance='" + amount.Text + "',openingDate='" + date.Value + "' where id='" + id + "'", conn).ExecuteNonQuery();
                    conn.Close();
                    conn.Open();
        ...
        public Boolean loadCustomer(string id)
        {
            try
            {
                db.setCursoerWait();
                conn.Open();
                reader = new SqlCommand("select * from liabilities where id='" + id + "'", conn).ExecuteReader();
                if (reader.Read())
                {
                    states = true;
 ...
 MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
            {
                try
                {
                    ...
                    else
                    {
                        conn.Close();
                        conn.Open();
                        new SqlCommand("insert into liabilities values ('" + number.Text + "','" + name.Text + "','" + amount.Text + "','" + date.Value + "')", conn).ExecuteNonQuery();
                        conn.Close();

                        conn.Open();
                        new SqlCommand("insert into accounts values('" + number.Text + "','" + name.Text + "','" + "LIABILITIES" + "')", conn).ExecuteNonQuery();
                        conn.Close();
                    }
                    conn.Close();
        ...
        private void number_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                conn.Open();
                reader = new SqlCommand("select * from liabilities ", conn).ExecuteReader();
                while (reader.Read())
                {
