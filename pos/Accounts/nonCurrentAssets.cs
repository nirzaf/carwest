...
namespace pos
{
    public partial class nonCurrentAssets : Form
    {
        ...
        public nonCurrentAssets(Form form, string user)
        {
            InitializeComponent();
            formH = form;
            userH = user;
        ...
        private void save()
        {
            try
            {
                ...
                else
                {
                    ...
                    conn.Open();
                    new SqlCommand("insert into receipt values('" + tempID + "','" + date.Value + "','" + "NON CURRENT ASSETS" + "','" + comboBox1.SelectedItem.ToString().Split('.')[0] + "','" + new amountByName().setAmountName(amountCash.Text) + "','" + amountCash.Text + "','" + payee.Text + "','" + "" + "','" + "" + "','" + userH + "','" + DateTime.Now + "')", conn).ExecuteReader();
                    conn.Close();

                    conn2.Open();
                    new SqlCommand("insert into cashSummery values('" + payee.Text + "/" + tempID + "','" + "NON CURRENT ASSETS-MANUAL" + "','" + Double.Parse(amountCash.Text) + "','" + date.Value.ToString() + "','" + userH + "')", conn2).ExecuteNonQuery();
                    conn2.Close();
        ...
        private void itemTable_Load(object sender, EventArgs e)
        {
            ...
            load();
            conn.Open();
            reader = new SqlCommand("select * from accounts where type='" + "NON CURRENT ASSETS" + "' and name!='" + "NON CURRENT ASSETS" + "'", conn).ExecuteReader();
            while (reader.Read())
            {
        ...
        private void load()
        {
            try
            {
                ...
                conn.Open();
                reader = new SqlCommand("select * from cashSummery where date='" + date.Value.ToShortDateString() + "' and remark='" + "NON CURRENT ASSETS-MANUAL" + "'", conn).ExecuteReader();
                while (reader.Read())
                {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.ColumnIndex == 3)
                {
                    ...
                    conn.Open();
                    new SqlCommand("delete from cashSummery where date='" + date.Value.ToShortDateString() + "' and remark='" + "NON CURRENT ASSETS-MANUAL" + "'", conn).ExecuteNonQuery();
                    conn.Close();
                    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                    {
                        conn.Open();
                        new SqlCommand("insert into cashSummery values('" + dataGridView1.Rows[i].Cells[0].Value + "','" + "NON CURRENT ASSETS-MANUAL" + "','" + dataGridView1.Rows[i].Cells[1].Value + "','" + date.Value.ToShortDateString() + "','" + userH + "')", conn).ExecuteNonQuery();
                        conn.Close();
                    }
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                dataGridView1.Rows.Clear();
                conn.Open();
                reader = new SqlCommand("select * from cashSummery where date between '" + dateFrom.Value.ToShortDateString() + "' and  '" + dateTo.Value.ToShortDateString() + "' and remark='" + "NON CURRENT ASSETS-MANUAL" + "'", conn).ExecuteReader();
                while (reader.Read())
                {
