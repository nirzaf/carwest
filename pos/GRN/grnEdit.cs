...
namespace pos
{
    public partial class grnEdit : Form
    {
        public grnEdit(Form form, String user, string id)
        {
            InitializeComponent();
            home = form;
            this.user = user;
    ...
    MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
      ...
      MessageBoxDefaultButton.Button1);
                        ...
                        {
                            newItemGRN ab = new newItemGRN(this);
        ...
        private double paidAmount, cashPaidDB;
        ...
        public void loadInvoice(string id)
        {
            try
            {
                ...
                if (reader.Read())
                {
                    ...
                    {
                        rowCount++;
                        dataGridView1.Rows.Add(rowCount, reader[1], reader[8], reader[5], "FALSE");
                    }
                    conn.Close();
                    ...
                    {
                        rowCount++;
                        dataGridView1.Rows.Add(rowCount, reader[1], reader[8], reader[5], "TRUE");
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    new itemTable5(this, "0", "0", dataGridView1.Rows[e.RowIndex].Cells[3].Value.ToString(), e.RowIndex + "", "0").Visible = true;
                }
            }
            catch (Exception a)
            {
        ...
        {
            //   new termXash5(this, Double.Parse(netTotal.Text), cashPaid, creditDetail, chequeDetail, cardDetail, creditDetailB, chequeDetailB, cardDetailB).Visible = true;
        }
...
MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
            {
                ...
                try
                {
                    ...
                    if (!creditDetailB & !chequeDetailB & !cardDetailB)
                    {
                        cashDetailB = true;
                        conn.Open();
                        new SqlCommand("update grn set supplierID='" + cutomerID + "',subTotal='" + netTotal.Text + "',idSupplier='" + grnNo.Text + "',userid='" + user + "',date='" + dateTimePicker1.Value + "',cash='" + netTotal.Text + "',paytype='" + "CASH- " + "' where id='" + invoieNoTemp + "'", conn).ExecuteNonQuery();
                        conn.Close();
                    }
                    else
                    {
                        cashDetailB = false;
                        conn.Open();
                        new SqlCommand("update grn set supplierID='" + cutomerID + "',subTotal='" + netTotal.Text + "',idSupplier='" + grnNo.Text + "',userid='" + user + "',date='" + dateTimePicker1.Value + "',cash='" + cashPaid + "',paytype='" + "CREDIT- " + "' where id='" + invoieNoTemp + "'", conn).ExecuteNonQuery();
                        conn.Close();
                    }
