...
namespace pos
{
    public partial class grnEdit2 : Form
    {
        public grnEdit2(Form form, String user, string id)
        {
            InitializeComponent();
            home = form;
            this.user = user;
        ...
        private string user, listBoxType, cutomerID = "", invoiceino, description, invoieNoTemp;
    ...
    MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
        ...
        private Boolean checkTerm()
        {
            ...
            {
                if (cashPaid == Double.Parse(payble.Text))
                {
                    creditDetailB = false;
                    chequeDetailB = false;
                    cardDetailB = false;
...
MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
            {
                ...
                try
                {
                    ...
                    {
                        conn.Open();
                        new SqlCommand("insert into creditGrn values ('" + invoieNoTemp + "','" + cutomerID + "','" + payble.Text + "','" + 0 + "','" + creditDetail[0] + "','" + creditDetail[1] + "','" + DateTime.Now + "')", conn).ExecuteNonQuery();
                        conn.Close();
                    }
                    if (chequeDetailB)
                    {
