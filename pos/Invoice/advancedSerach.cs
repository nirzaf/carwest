...
namespace pos
{
    public partial class advancedSerach : Form
    {
        public advancedSerach(Form form, String user)
        {
            InitializeComponent();
            this.user = user;
            home = form;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (checkBox1.Checked)
                {
                    reader = new SqlCommand("select a.* from advancedEmp as a  where a.date between '" + from.Value.ToShortDateString() + "' and '" + toDate.Value.ToShortDateString() + "'", conn).ExecuteReader();

                }
                else
                {
                    reader = new SqlCommand("select * from advancedEmp ", conn).ExecuteReader();
                ...
                while (reader.Read())
                {
                    ...
                    if (reader2.Read())
                    {
                        ...
                        dataGridView1.Rows.Add(reader[0], reader.GetDateTime(3).ToShortDateString(), vehicleNO, db.setAmountFormat(reader[2] + ""), 0, 0);
...
MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
                    {
                        ...
                        conn.Open();
                        new SqlCommand("delete from advancedEmp where id='" + invoieNoTemp + "'", conn).ExecuteNonQuery();
                        conn.Close();
                        cashB = 0;
                        for (int i = 0; i < dataGridView1.Rows.Count; i++)
                        {
                            cashB = cashB + Double.Parse(dataGridView1.Rows[e.RowIndex].Cells[3].Value.ToString());
                        }
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (checkBox1.Checked)
                {
                    reader3 = new SqlCommand("select DISTINCT a.id from advancedEmp as a ,emp as b where a.date between '" + from.Value.ToShortDateString() + "' and '" + toDate.Value.ToShortDateString() + "' and a.empid = '" + tempCustomer + "' ", conn3).ExecuteReader();


                }
                else
                {
                    reader3 = new SqlCommand("select DISTINCT a.id from advancedEmp as a ,emp as b where  a.empid = '" + tempCustomer + "' ", conn3).ExecuteReader();
                ...
                while (reader3.Read())
                {
                    try
                    {
                        ...
                        reader = new SqlCommand("select * from advancedEmp where id='" + reader3[0] + "'", conn).ExecuteReader();
        ...
        public Boolean loadCustomer(string id)
        {
            ...
            try
            {
                ...
                {
                    reader = new SqlCommand("select name from emp where empid='" + id + "'", conn).ExecuteReader();
        ...
        private void cutomerUnSaved_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (!(e.KeyValue == 12 | e.KeyValue == 13 | cutomerUnSaved.Text.Equals("")))
            {
                ...
                try
                {
                    ...
                    if (radioCustomer.Checked)
                    {
                        reader = new SqlCommand("select empid,name from emp where name like '%" + cutomerUnSaved.Text + "%' ", conn).ExecuteReader();
