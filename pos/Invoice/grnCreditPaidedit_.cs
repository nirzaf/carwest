...
namespace pos
{
    public partial class grnCreditPaidedit_ : Form
    {
        public grnCreditPaidedit_(Form home, String user)
        {
            InitializeComponent();
            formH = home;
            userH = user;
        ...
        void loadInvoice()
        {
            try
            {
                ...
                if (searchALL.Checked)
                {
                    ...
                    conn.Open();
                    reader = new SqlCommand("select a.invoiceid,b.id,b.company,a.balance,a.date from creditgrn as a,supplier as b " + queary, conn).ExecuteReader();
                    while (reader.Read())
                    {
                        try
                        {
                            ...
                            if (radioSettele.Checked)
                            {
                                ...
                                {
                                    dataGridView3.Rows.Add(reader[1] + " " + reader[2], reader[0], db.setAmountFormat(reader[3] + ""), db.setAmountFormat(tempPaid + ""), reader.GetDateTime(4).ToShortDateString(), db.setAmountFormat((reader.GetDouble(3) - tempPaid) + ""), true);
                                }
                            }
                            else if (radioPending.Checked)
                            {
                                if (tempPaid < reader.GetDouble(3))
                                {
                                    dataGridView3.Rows.Add(reader[1] + " " + reader[2], reader[0], db.setAmountFormat(reader[3] + ""), db.setAmountFormat(tempPaid + ""), reader.GetDateTime(4).ToShortDateString(), db.setAmountFormat((reader.GetDouble(3) - tempPaid) + ""), false);
                                }
                            }
                        }
                        catch (Exception)
                ...
                else if (radioAdvancedSearch.Checked)
                {
                    ...
                    {
                        conn.Open();
                        reader = new SqlCommand("select a.invoiceid,b.id,b.company,a.balance,a.date from creditgrn as a,customer as b where a.customerid='" + dataGridView2.Rows[i].Cells[0].Value + "' " + queary, conn).ExecuteReader();
                        while (reader.Read())
                        {
                            try
                            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (searchALL.Checked)
                {
                    ...
                    conn.Open();
                    reader = new SqlCommand("select a.* from receipt2 as a " + queary, conn).ExecuteReader();
                    ...
                    while (reader.Read())
                    {
                        ...
                        {
                            conn2.Open();
                            reader2 = new SqlCommand("select id,name,company from supplier where id='" + reader[3] + "'", conn2).ExecuteReader();
                            if (reader2.Read())
                            {
                ...
                else if (radioAdvancedSearch.Checked)
                {
                    ...
                    {
                        conn.Open();
                        reader = new SqlCommand("select a.* from receipt2 as a where a.customer='" + dataGridView2.Rows[i].Cells[0].Value + "' " + queary, conn).ExecuteReader();
                        //  MessageBox.Show("sasa");
...
MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
                    {
                        ...
                        conn.Open();
                        new SqlCommand("delete from grnCreditPaid where tempid='" + dataGridView1.Rows[e.RowIndex].Cells[3].Value + "'", conn).ExecuteNonQuery();
                        conn.Close();
                        conn.Open();
                        new SqlCommand("delete from grnCreditPaid where tempid='" + "OP/" + dataGridView1.Rows[e.RowIndex].Cells[3].Value + "'", conn).ExecuteNonQuery();
                        conn.Close();
                        conn.Open();
                        new SqlCommand("delete from overPayS where tempid='" + dataGridView1.Rows[e.RowIndex].Cells[3].Value + "'", conn).ExecuteNonQuery();
                        conn.Close();
                        conn.Open();
                        ...
                        conn.Close();
                        conn.Open();
                        new SqlCommand("delete from supplierStatement where reason='" + "SETTELE-" + dataGridView1.Rows[e.RowIndex].Cells[3].Value + "'", conn).ExecuteNonQuery();
                        conn.Close();
                        conn.Open();
                        ...
                        conn.Close();
                        conn.Open();
                        new SqlCommand("delete from cashSummery where reason='" + "GRN Credit Paid-" + dataGridView1.Rows[e.RowIndex].Cells[3].Value + "'", conn).ExecuteNonQuery();
                        conn.Close();
        ...
        private void pRINTPREVIEWToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                    {
                        if (dataGridView1.Rows[i].Cells[4].Value.ToString().Equals("CHEQUE"))
                        {
                            conn.Open();
