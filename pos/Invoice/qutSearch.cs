...
namespace pos
{
    public partial class qutSearch : Form
    {
        public qutSearch(Form form, String user)
        {
            InitializeComponent();
            this.user = user;
            home = form;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                query = "qua";
                conn.Open();
                reader = new SqlCommand("select a.id,a.subTotal,a.date,a.customerId from " + query + " as a  where a.date between '" + from.Value.ToShortDateString() + "' and '" + toDate.Value.ToShortDateString() + "'", conn).ExecuteReader();
                while (reader.Read())
                {
                    conn2.Open();
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    conn3.Open();
                    reader3 = new SqlCommand("select DISTINCT a.id from " + query + " as a ,customer as b where a.date between '" + from.Value.ToShortDateString() + "' and '" + toDate.Value.ToShortDateString() + "' and a.customerid = '" + tempCustomer + "' ", conn3).ExecuteReader();
                    while (reader3.Read())
                    {
                        conn.Open();
                ...
                {
                    conn3.Open();
                    reader3 = new SqlCommand("select DISTINCT a.id from " + query + " as a ,customer as b where  a.customerid = '" + tempCustomer + "' ", conn3).ExecuteReader();
                    while (reader3.Read())
                    {
                        conn.Open();
                        reader = new SqlCommand("select a.id,a.subTotal,a.date,a.customerId from " + query + " as a  where a.customerId='" + tempCustomer + "' and a.id='"+reader3[0]+"' ", conn).ExecuteReader();
