...
namespace pos
{
    public partial class returnGood : Form
    {
        public returnGood(Form form, String user, string invoiceType, string invoino)
        {
            InitializeComponent();
            home = form;
            this.user = user;
                    ...
                    if (item)
                    {
                        ...
                        else
                        {
...

                            {
                                ...
                                dataGridView1.Rows.Add(rowCount + "", code.Text, description, unitPrice.Text, discount.Text, costPrice.Text, qtyOut.Text, qtyOut.Text, amount, radioButton1.Checked);
    ...
    MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
        ...
        private void button1_Click_2(object sender, EventArgs e)
        {
            ...
            returnSearch a = new returnSearch(this, user);
            a.Visible = true;
            a.TopMost = true;
        }
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                reader = new SqlCommand("select * from invoiceRetailDetail where invoiceID='" + invoiceID.Text + "' and itemCode='" + dataGridView1.Rows[0].Cells[1].Value.ToString() + "'", conn).ExecuteReader();
                if (reader.Read())
                {
                    states = true;
                    tempInvoiceID = "R-" + reader[0] + "";
                }
                else
                {
                    MessageBox.Show("Sorry, Invaled Invoice ID or Item not Exit on Enterd Invoice");
                }
                conn.Close();
            ...
            {
                conn.Open();
                reader = new SqlCommand("select * from grnDetail where invoiceID='" + invoiceID.Text + "' and itemCode='" + dataGridView1.Rows[0].Cells[1].Value.ToString() + "'", conn).ExecuteReader();
                if (reader.Read())
                {
                    states = true;
                    tempInvoiceID = reader[0] + "";
                }
                else
                {
                    MessageBox.Show("Sorry, Invaled GRN ID or Item not Exit on Enterd Invoice");
                }
                conn.Close();
            ...
            if (states)
            {
                ...
                try
                {
                    ...
                    conn.Open();
                    new SqlCommand("delete from cashSummery where reason='" + "Return Good" + "' and remark='" + "Return No-" + invoieNoTemp + "'", conn).ExecuteNonQuery();
                    conn.Close();
                    ...
                    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                    {
                        ...
                        if (dataGridView1.Rows[i].Cells[9].Value.ToString().ToUpper().Equals("TRUE"))
                        {
                            ...
                            conn.Close();
                            conn2.Open();
                            new SqlCommand("insert into returnGoods values ('" + dataGridView1.Rows[i].Cells[1].Value + "','" + Double.Parse(dataGridView1.Rows[i].Cells[6].Value.ToString()) + "','" + cutomerID + "','" + false + "','" + Double.Parse(dataGridView1.Rows[i].Cells[8].Value.ToString()) + "','" + DateTime.Now + "','" + "RE-" + invoieNoTemp + "','" + tempInvoiceID + "')", conn2).ExecuteNonQuery();
                            conn2.Close();
                        ...
                        {
                            conn.Open();
                            new SqlCommand("update item set qty=qty-'" + Double.Parse(dataGridView1.Rows[i].Cells[7].Value.ToString()) + "' where code='" + dataGridView1.Rows[i].Cells[1].Value.ToString() + "'", conn).ExecuteNonQuery();
                            ...
                            conn.Close();
                            conn2.Open();
                            new SqlCommand("insert into returnGoods values ('" + dataGridView1.Rows[i].Cells[1].Value + "','" + Double.Parse(dataGridView1.Rows[i].Cells[6].Value.ToString()) + "','" + cutomerID + "','" + true + "','" + Double.Parse(dataGridView1.Rows[i].Cells[8].Value.ToString()) + "','" + DateTime.Now + "','" + "RE-" + invoieNoTemp + "','" + tempInvoiceID + "')", conn2).ExecuteNonQuery();
                            conn2.Close();
                        }
