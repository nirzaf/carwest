...
namespace pos
{
    ...
    {
        public customerReport(Form home, String user)
        {
            InitializeComponent();
            formH = home;
            userH = user;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                conn.Open();
                reader = new SqlCommand("select * from customer  " + queary + "", conn).ExecuteReader();
                while (reader.Read())
                {
        ...
        private void name_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                conn.Open();
                dataGridView1.Rows.Clear();
                reader = new SqlCommand("select * from customer where description like '%" + name.Text + "%' " + queary + "", conn).ExecuteReader();
                while (reader.Read())
                {
                    dataGridView1.Rows.Add(reader.GetString(0).ToUpper(), reader.GetString(1).ToUpper(), reader.GetString(2).ToUpper(), reader.GetString(3).ToUpper(), reader.GetString(4).ToUpper(), reader.GetString(5).ToUpper(), reader.GetString(6).ToUpper(), reader.GetString(7).ToUpper());
                }
