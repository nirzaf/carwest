...
namespace pos
{
    public partial class stockOrderHistorey : Form
    {
        public stockOrderHistorey(Form home, String user)
        {
            InitializeComponent();
            formH = home;
            userH = user;
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            try
            {
                conn.Open();
                dataGridView1.Rows.Clear();
                reader = new SqlCommand("select a.*,b.company from stockOrder as a,supplier as b where a.date between '" + dateFrom.Value.ToShortDateString() + " 00:00" + "' and '" + dateTo.Value.ToShortDateString() + " 23:59:59" + "' and a.company=b.id ", conn).ExecuteReader();
                while (reader.Read())
                {
                    dataGridView1.Rows.Add(reader.GetDateTime(7).ToShortDateString(), reader[1] + " " + reader[2], reader[4], reader[8], reader[6]);
                }
                conn.Close();
