...
namespace pos
{
    ...
    {
        public stockReport(Form home, String user)
        {
            InitializeComponent();
            formH = home;
            userH = user;
            ...
            else
            {
                try
                {
                    ...
                    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                    {
                        ...
                        {
                            conn.Open();
                            reader = new SqlCommand("select retailPrice,QTY,categorey from item where code='" + dataGridView1.Rows[i].Cells[0].Value + "'", conn).ExecuteReader();
                            if (reader.Read())
                            {
                    ...
                    // MessageBox.Show("a");
                    stockReportALL_ pp = new stockReportALL_();
                    pp.SetDataSource(ds);
            ...
            else
            {
                try
                {
                    ...
                    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                    {
                        ...
                        {
                            conn.Open();
                            reader = new SqlCommand("select retailPrice,QTY,categorey,purchasingPrice from item where code='" + dataGridView1.Rows[i].Cells[0].Value + "'", conn).ExecuteReader();
                            if (reader.Read())
                            {
