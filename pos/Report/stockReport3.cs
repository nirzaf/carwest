...
namespace pos
{
    public partial class stockReport3 : Form
    {
        public stockReport3(Form home, String user)
        {
            InitializeComponent();
            formH = home;
            userH = user;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                while (reader.Read())
                {
                    totValue=totValue+(reader.GetDouble(9)*reader.GetDouble(6));
                    dataGridView1.Rows.Add(reader.GetString(0).ToUpper(), reader[9] + " " + reader.GetString(5).ToUpper(), reader.GetString(1).ToUpper(), reader.GetString(2).ToUpper(), reader.GetString(3).ToUpper(), db.setAmountFormat(reader[6] + ""), db.setAmountFormat(reader.GetDouble(9) * reader.GetDouble(6) + ""));
                }
                conn.Close();

                totalStockValue.Text = db.setAmountFormat(totValue+"");
