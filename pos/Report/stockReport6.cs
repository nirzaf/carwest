...
namespace pos
{
    public partial class stockReport6 : Form
    {
        public stockReport6(Form home, String user)
        {
            InitializeComponent();
            formH = home;
            userH = user;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    conn.Open();
                    reader = new SqlCommand("select * from chequeInvoiceRetail where chequeDate='" + dateTimePicker1.Value.ToShortDateString() + "'", conn).ExecuteReader();
                    while (reader.Read())
                    {
                        total = total + reader.GetDouble(4);
                        dataGridView1.Rows.Add("R-" + reader[0], reader.GetDateTime(7).ToShortDateString(), reader[5], reader[8], db.setAmountFormat(reader[4] + ""));
                    }
                    conn.Close();
                    conn.Open();
                    reader = new SqlCommand("select * from chequeInvoiceRetail2 where chequeDate='" + dateTimePicker1.Value.ToShortDateString() + "'", conn).ExecuteReader();
                    while (reader.Read())
                    {
                        total = total + reader.GetDouble(4);
                        dataGridView1.Rows.Add("R-" + reader[0], reader.GetDateTime(7).ToShortDateString(), reader[5], reader[8], db.setAmountFormat(reader[4] + ""));
                ...
                {
                    conn.Open();
                    reader = new SqlCommand("select * from chequeGRN where chequeDate='" + dateTimePicker1.Value.ToShortDateString() + "'", conn).ExecuteReader();
                    while (reader.Read())
                    {
                        total = total + reader.GetDouble(4);
                        dataGridView1.Rows.Add(reader[0], reader.GetDateTime(7).ToShortDateString(), reader[5], reader[8], db.setAmountFormat(reader[4] + ""));
                    }
                    conn.Close();
                }
                ...
                textBox1.Text = db.setAmountFormat(total + "");
        ...
        private double credit = 0, debit = 0, stock = 0, stockDB = 0;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            conn.Open();
            reader = new SqlCommand("select * from itemStatementStock where  date='" + DateTime.Now + "'", conn).ExecuteReader();
            while (reader.Read())
            {
                conn2.Open();
