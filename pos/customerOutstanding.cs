...
namespace pos
{
    ...
    {
        public customerOutstanding(Form home, String user)
        {
            InitializeComponent();
            formH = home;
            userH = user;
            ...
            try
            {
                ...
                if (radioAllItem.Checked)
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        {
                            conn2.Open();
                            reader2 = new SqlCommand("select paid,balance from invoiceCreditPaid where invoiceID='" + reader[0] + "'", conn2).ExecuteReader();
                            while (reader2.Read())
                            {
