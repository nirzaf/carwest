...
namespace pos
{
    public partial class invoiceCheque2 : Form
    {
        public invoiceCheque2(Form home, String user)
        {
            InitializeComponent();
            formH = home;
            userH = user;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (searchALL.Checked)
                {
                    ...
                    if (isCompany)
                    {
                        reader = new SqlCommand("select a.invoiceID,a.amount,a.paid,a.balance,a.date,b.customerid,a.id from invoiceCreditPaid as a,invoiceRetail as b where b.id=a.invoiceId " + queary, conn).ExecuteReader();

                    }
                    else {
                        reader = new SqlCommand("select a.invoiceID,a.amount,a.paid,a.balance,a.date,b.customerid,a.id from invoiceCreditPaid as a,invoiceDump as b where b.id=a.invoiceId " + queary, conn).ExecuteReader();
                    ...
                    while (reader.Read())
                    {
                      //  MessageBox.Show("sa");
                        try
                        {
                            ...
                            if (reader2.Read())
                            {
                                dataGridView1.Rows.Add(reader2[0].ToString().ToUpper() + " " + reader2[1].ToString().ToUpper() + " " + reader2[2].ToString().ToUpper(), "R-" + reader[0], db.setAmountFormat(reader[1] + ""), db.setAmountFormat(reader[2] + ""), db.setAmountFormat(reader[3] + ""), reader.GetDateTime(4).ToShortDateString(), reader[6]);

                            }
                            else
                            {
                                dataGridView1.Rows.Add(reader[5], "R-" + reader[0], db.setAmountFormat(reader[1] + ""), db.setAmountFormat(reader[2] + ""), db.setAmountFormat(reader[3] + ""), reader.GetDateTime(4).ToShortDateString(), reader[6]);
                ...
                else if (radioAdvancedSearch.Checked)
                {
                    for (int i = 0; i < dataGridView2.Rows.Count; i++)
                    {
                        ...
                        if (isCompany)
                        {
                            reader = new SqlCommand("select a.invoiceID,a.amount,a.paid,a.balance,a.date,b.customerid,a.id from invoiceCreditPaid as a,invoiceRetail as b where b.id=a.invoiceId and b.customerid='" + dataGridView2.Rows[i].Cells[0].Value + "'" + queary, conn).ExecuteReader();

                        }
                        else {
                            reader = new SqlCommand("select a.invoiceID,a.amount,a.paid,a.balance,a.date,b.customerid,a.id from invoiceCreditPaid as a,invoiceDump as b where b.id=a.invoiceId and b.customerid='" + dataGridView2.Rows[i].Cells[0].Value + "'" + queary, conn).ExecuteReader();
...
MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
                    {
                        var a = 0.0;
                        conn.Open();
                        reader = new SqlCommand("select amount from invoiceCreditPaid where id='" + dataGridView1.Rows[e.RowIndex].Cells[6].Value + "'", conn).ExecuteReader();
                        if (reader.Read())
                        {
                        ...
                        conn.Close();
                        conn.Open();
                        new SqlCommand("delete from invoiceCreditPaid where id='" + dataGridView1.Rows[e.RowIndex].Cells[6].Value + "'", conn).ExecuteNonQuery();
                        conn.Close();

                        conn.Open();
                        reader = new SqlCommand("select * from invoiceCreditPaid where date='" + DateTime.Now.ToShortDateString() + "' and id='" + dataGridView1.Rows[e.RowIndex].Cells[6].Value + "'", conn).ExecuteReader();
                        if (reader.Read())
                        {
                            conn.Close();
                            conn.Open();
                            new SqlCommand("insert into cashSummery values ('" + "Invoice Credit Paid-Settle" + "','" + "Delete Record above Invoice ID-" + dataGridView1.Rows[e.RowIndex].Cells[1].Value + "','" + -a + "','" + DateTime.Now + "','" + userH + "')", conn).ExecuteNonQuery();
                            conn.Close();
                        }
                        else
                        {
                            if ((MessageBox.Show("This Payment not a To-Day Settelmnt Record, Do You Need to Balance Cash-Flow on To-Day?", "Confirmation",
MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question,
MessageBoxDefaultButton.Button1) == System.Windows.Forms.DialogResult.Yes))
                            {
                                conn.Close();
                ...
                {
                    //this.Enabled = false;
                    new invoiceCreditPayEdit(this, userH, dataGridView1.Rows[e.RowIndex].Cells[1].Value.ToString().Split('-')[1], dataGridView1.Rows[e.RowIndex].Cells[6].Value.ToString(),e.RowIndex+"").Visible = true;
                }
