...
namespace pos
{
    public partial class supplierOutstanding2 : Form
    {
        public supplierOutstanding2(Form home, String user)
        {
            InitializeComponent();
            formH = home;
            userH = user;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    conn2.Open();
                    reader2 = new SqlCommand("select distinct(customerID) from supplierStatement", conn2).ExecuteReader();
                    while (reader2.Read())
                    {
                        amountCost = 0;
                        amountPaid = 0;
                        ...
                        {
                            conn.Open();
                            reader = new SqlCommand("select sum(credit) from supplierStatement where states='" + true + "' and customerID='" + reader2[0] + "'", conn).ExecuteReader();
                            if (reader.Read())
                            {
                        ...
                        {
                            conn.Open();
                            reader = new SqlCommand("select sum(debit) from supplierStatement where states='" + true + "' and customerID='" + reader2[0] + "'", conn).ExecuteReader();
                            if (reader.Read())
                            {
                        ...
                        conn.Open();
                        reader = new SqlCommand("select id,name,company from supplier where id='" + reader2[0] + "'", conn).ExecuteReader();
                        if (reader.Read())
                        {
                ...
                else if (radioAdvancedSearch.Checked)
                {
                    for (int i = 0; i < dataGridView2.Rows.Count; i++)
                    {
                        ...
                        {
                            conn.Open();
                            reader = new SqlCommand("select sum(credit) from supplierStatement where states='" + true + "' and customerID='" + dataGridView2.Rows[i].Cells[0].Value + "'", conn).ExecuteReader();
                            if (reader.Read())
                            {
                                amountCost = reader.GetDouble(0);
                            }
                        ...
                        {
                            conn.Open();
                            reader = new SqlCommand("select sum(debit) from supplierStatement where states='" + true + "' and customerID='" + dataGridView2.Rows[i].Cells[0].Value + "'", conn).ExecuteReader();
                            if (reader.Read())
                            {
                                amountPaid = reader.GetDouble(0);
                            }
                        ...
                        conn.Open();
                        reader = new SqlCommand("select id,name,company from supplier where id='" + dataGridView2.Rows[i].Cells[0].Value + "'", conn).ExecuteReader();
                        if (reader.Read())
                        {
                            dataGridView1.Rows.Add(reader[0].ToString().ToUpper() + " " + reader[1].ToString().ToUpper() + " " + reader[2].ToString().ToUpper(), amountCost, amountPaid, Math.Round((amountCost - amountPaid), 2));
                        }
                        ...
                        {
                            dataGridView1.Rows.Add(dataGridView2.Rows[i].Cells[0].Value, amountCost, amountPaid, Math.Round((amountCost - amountPaid), 2));
                        }
                        conn.Close();
                    }
                }
        ...
        private customerStatement statmentH;
